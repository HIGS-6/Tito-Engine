...
class TopDownCharacterController(Sprite):
    ...
    def move(self):
        ...
        x_input, y_input = InputManager.get_axis(pg.K_a, pg.K_d), InputManager.get_axis(pg.K_w, pg.K_s)

        self.transform.position += Vec2(x_input, y_input).normalize() * self._move_speed

    def look(self):
        # Calculate the look angle and rotate the sprite
        mouse_pos = InputManager.get_mouse_pos()

        rel_x, rel_y = mouse_pos.x - self.transform.position.x, mouse_pos.y - self.transform.position.y
